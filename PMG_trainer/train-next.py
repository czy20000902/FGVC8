...
def train(nb_epoch, batch_size, store_name, resume=False, start_epoch=0, model_path=None):
    ...
    lrb = 0.00002
    optimizer = optim.SGD([
        {'params': net.classifier_concat.parameters(), 'lr': lra},
        {'params': net.conv_block1.parameters(), 'lr': lra},
        {'params': net.classifier1.parameters(), 'lr': lra},
        {'params': net.conv_block2.parameters(), 'lr': lra},
        {'params': net.classifier2.parameters(), 'lr': lra},
        {'params': net.conv_block3.parameters(), 'lr': lra},
        {'params': net.classifier3.parameters(), 'lr': lra},
        {'params': net.features.parameters(), 'lr': lrb}
    ...
    for epoch in range(start_epoch, nb_epoch):
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            # Step 1
            optimizer.zero_grad()
            inputs1 = jigsaw_generator_2(inputs, 8)
            ...
            # Step 2
            optimizer.zero_grad()
            inputs2 = jigsaw_generator_2(inputs, 8)
            ...
            # Step 3
            optimizer.zero_grad()
            inputs3 = jigsaw_generator_2(inputs, 8)
        ...
        if epoch < 5 or epoch >= 80 or True:
            ...
            torch.save(net, './checkpoints/%d.pth'%(epoch))
        else:
            net.cpu()
            torch.save(net, './' + store_name + '/model.pth')
            net.to(device)
...
train(nb_epoch=200,             # number of epoch
         ...
         model_path="checkpoints/199.pth")         # the saved model where you want to resume the training
