...
def load_model(model_name, pretrain=True, require_grad=True):
    ...
    if model_name == 'resnet101':
        net = ptcv_get_model("resnet101", pretrained=True)
        net.output = torch.nn.Linear(2048, 2048)
        for param in net.parameters():
            param.requires_grad = require_grad
        net = PMG(net, 512, 810)
    ...
    if model_name == 'resnet152':
        net = ptcv_get_model("resnet152", pretrained=True)
        net.output = torch.nn.Linear(2048, 2048)
        for param in net.parameters():
            param.requires_grad = require_grad
        net = PMG(net, 512, 810)
    ...
    if model_name == 'efficientnet_b1':
        net = ptcv_get_model("efficientnet_b1", pretrained=True)
        net.output.fc = torch.nn.Linear(1280, 2048)
        for param in net.parameters():
            param.requires_grad = require_grad
        net = PMG(net, 512, 810)
    ...
    if model_name == 'efficientnet_b2c':
        net = ptcv_get_model("efficientnet_b2c", pretrained=True)
    ...
    if model_name == 'efficientnet_b3b':
        net = ptcv_get_model("efficientnet_b3b", pretrained=True)
...
def test(net, criterion, batch_size, input_size):
    ...
    testset = torchvision.datasets.ImageFolder(root='/userhome/21inat/val/',
